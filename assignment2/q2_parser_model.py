...
class ParserModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        feed_dict = {
            ...
            self.labels_placeholder: labels_batch if labels_batch is not None else np.zeros([1,3]),
    ...
    def add_embedding(self):
        ...
        input_embbeded = tf.nn.embedding_lookup(emb, self.input_placeholder)
        embeddings = tf.reshape(input_embbeded, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
